...
def interactiveInterface():
    ...
    hbox_play = wg.HBox([frame_n, slider])

    hbox = wg.HBox([json_dropdown, hbox_play])
...
def edit_video(video_name, n, r, x=[0,-1], y=[0,-1], in_height=-1, save_vid=False, output_path='None'):
    n = int(np.round(n))
    if(video_name == "None"):
        print("Choose a video")
    else:
...
def interactiveInterface_1():
    ...
    tabs = ['Choose Video', 'Edit Video', 'Analyze Data']
    children = []
    children.append(vbox_video)
    children.append(manipulate_video())
    children.append(hbox_res)
    tab = wg.Tab()
    tab.children = children
    for i in range(len(children)):
